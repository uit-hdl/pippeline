...
produceDocumenationAndData <- function( scriptFile, docFile) {
  # pandoc'ing with target PDF is buggy (R 3.2.3, rmarkdown 1.4, pandoc 1.16.0.2)
...
generatePipeline <- function( params) {
  ...
  readStep <- createStep( 'Datasets', TRUE, c(
    cmt( 'Reading in datasets'),
  ...
  exclStep <- createStep( 'Transitions', input$trans, c(
    cmt( 'Exclusion of control-case transitions')
  ) )
  outlStep <- createStep( 'Outliers', input$outlierEnabled, c(
    cmt( 'Removal of outliers')
  ) )
