...
params$path <- "/pippelinen/nowaclean_outliers" # your pippeline output folder
params$experiment <- "" # dataset name
...
# outlierObj <- setClass('OutlierCandidate', slots =
# c(sample='character', descr='character')) outlierObj <-
# list(sample='', descr='')
...
getLabInfoBySample <- function(sID, experiment, sameAsThreshhold) {
  if (!is.null(params$overviewVars)) {
    ...
    if (sameAsThreshhold) 
      result <- result[, params$overviewVars]
    return(result)
  } else {
...
readTF <- function(strprmt) {
  ...
  n <- tolower(n)
  if (n == "yes" || n == "y") 
    return(T) else if (n == "no" || n == "n") 
      return(F) else n <- readTF(strprmt)
}
