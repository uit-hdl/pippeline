...
# pipeline: basics
basics <- list(appName = "Pippeline", 
               title = "Biobank Dataset Processing Script & Report", 
               optionsFile = "options.csv", 
               questsFile = "questionnaires.txt",
               docFormat = "html",  # note: pandoc'ing with target PDF is buggy in R 3.2.3, rmarkdown 1.4, pandoc 1.16.0.2
               msgDuration = 7, 
               ids = c("lpnr", "labnr", "serienr")) # ID labels to remove before data are written



# tabs
critTabsId <- c("outliers", "corr", "filter", "norm", "quest", "process", 
                "divd")
...
tryCatch({
  ...
  showNotification(paste0("Error while loading file ", basics$optionsFile, 
                          ". (Wrong format?) Error code #5.", type = "error", duration = basics$msgDuration))
})
...
tryCatch({
  ...
  showNotification(paste0("Error while loading file ", basics$questsFile, 
                          ". (Wrong format?) Error code #6.", type = "error", duration = basics$msgDuration))
})
