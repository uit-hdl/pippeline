...
observeEvent(input$descrNext, {
  ...
  tmpDataScriptVec <<- c(file.path(tmpFolder, "tmpData.rds"), file.path(tmpFolder, 
                                                                        "tmpPipeline.R"))
  ...
  tryCatch({
    ...
    showNotification("Temporary files will be written to: ", toString(tmpFolder), 
...
observeEvent(input$steps, {
  if (input$steps == "design") {
    resetStepsAndInfo()
    showNotification("Resetting project options...", type = "warning", 
                     duration = 3)
  }
})
...
observeEvent(input$designDown, {
  # infoPairs <- interStepAndUpdate(tmpDataScriptVec)
  # piplInfo$currFeatures <<- infoPairs[1] piplInfo$currSamples <<-
  # infoPairs[2] origPairs <<- infoPairs
...
observeEvent(input$outlierApply, {
  ...
  outlierStatus <- (input$outlierEnabled && input$outlierFile != notSelOpt && 
                      outlFileExists()) || !input$outlierEnabled
  transStatus <- (input$transEnabled && input$cctFile != notSelOpt && 
                    cctFileExists()) || !input$transEnabled
  if (outlierStatus && transStatus) 
    applyAction()
})
...
# observeEvent(input$outlierBack, { updateNavlistPanel(session,
# 'steps', selected = 'design') })
...
observeEvent(input$normApply, {
  ...
  # normStatus <- FALSE if (input$nmeth == 'vstQuantileNorm') normStatus
  # <- (input$normEnabled && input$nmeth != notSelOpt) ||
  # !input$normEnabled if (input$nmeth == 'ComBat') normStatus <-
  # (input$normEnabled && input$nmeth != notSelOpt && input$batchTab !=
  # notSelOpt && input$batchVar != notSelOpt) || !input$normEnabled if
  # (normStatus) applyAction()
  if (methodIsValid()) 
    applyAction()
})
...
observeEvent(input$batchTab, {
  if (input$batchTab != notSelOpt) 
    btchvar <<- c(btchvar, getDSColNames(input$batchTab)) else btchvar <<- c(notSelOpt)
...
observeEvent(input$questApply, {
  ...
  questStatus <- (input$questEnabled && input$questObj != notSelOpt && 
                    length(input$questVars) > 0) || !input$questEnabled
  if (questStatus) 
    applyAction()
})
...
# observeEvent(input$steps, { if (input$steps == 'process') { infoPairs
# <- interStepAndUpdate(tmpDataScriptVec) piplInfo$currFeatures <<-
# infoPairs[1] piplInfo$currSamples <<- infoPairs[2] if
# (input$designDown) origPairs <<- infoPairs } }
# #-------------------------
...
observeEvent(input$wantGenes, {
  if (as.logical(input$wantGenes) && !as.logical(input$normEnabled)) 
    showNotification("Conversion to samples implies taking the average of multiple probes.
                     You may thus want to consider to enable normalization.", 
                     type = "warning")
})

# Quit
observeEvent(input$quit, {
  showModal(modalDialog(title = "Quit", 
                        paste0("Really quit ", basics$appName, "?"), 
                        size = "s", 
                        footer = tagList(actionButton("reallyQuit", "Yes, quit"), 
...
observeEvent(input$process, {
  # setting the filename works currently only when run in an external
  # browser window, but not in Rstudio window/viewer pan
  ts <<- Sys.time()
  showNotification("Processing. This may take some time. Please stand by ..", 
                   duration = NULL, id = "wait")
  
  procFolder <<- gsub("//", "/", file.path(pipProjects, paste0(input$projname, 
                                                               "-", format(startTime, "%d%m%Y-%H%M%OS3"))))
  ...
  pipeline <- generatePipeline(list(sourceObjs = sourceObjs(), targetFile = dataFile))
  # fill files with content
  tryCatch({
    # Copy outliers report
    if (file.exists(file.path(nowacleanReports, input$outlierFileReport)) && 
        input$outlierFileReport != notSelOpt && input$outlierEnabled) {
    ...
    if (file.exists(file.path(cctransReports, input$transFileReport)) && 
        input$transFileReport != notSelOpt && input$transEnabled) {
    ...
    render(scriptFile, paste0(basics$docFormat, "_document"), docFile, 
           quiet = TRUE)
    # render( scriptFile) showNotification('Successful render', scriptFile,
    ...
    removeNotification("wait")
    showNotification("Could not produce data/documentation. (Error while sourcing script.) Error code #3.", 
                     type = "error", duration = NULL)
    showNotification("Error info: ", toString(err), type = "error", 
                     duration = NULL)
  })
  
  # tryCatch({ unlink(tmpFolder, recursive=TRUE) }, warning =
  # function(wrn){ NULL })
  ...
  tryCatch({
    ...
    system("git init")
    system("git add .")
    system(sprintf("git commit -m'%s'", paste0("First commit for pippelinen repo - ", 
                                               format(startTime, "%d%m%Y-%H%M%OS3"))))
    
    print(tmpDir)
    # Add R-project file: TODO
  }, warning = function(wrn) {
    NULL
  })
